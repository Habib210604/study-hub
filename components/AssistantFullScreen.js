...
import { Bot, Send, Paperclip, Sparkles, Camera, Plus, Trash2, CheckSquare, Square } from 'lucide-react';
...
export default function AssistantFullScreen() {
  ...
  return (
    <div className="flex h-[75vh] gap-4">
      {/* --- History sidebar --- */}
      <div className="w-56 shrink-0 border-r border-[var(--border)] pr-4 flex flex-col">
        <button
          onClick={() => startNewChat()}
          className="flex items-center gap-2 text-sm font-medium bg-teal-500/10 hover:bg-teal-500/20 text-teal-400 px-3 py-2 rounded-lg transition cursor-pointer mb-3"
        >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center gap-2 pb-4 border-b border-[var(--border)] mb-4">
          <Bot size={20} className="text-teal-400" />
          <h3 className="font-semibold text-[var(--text)]">Study AI Assistant</h3>
        </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 px-1">
          {loadingMessages ? (
            ...
            <div className="text-center text-[var(--text-muted)] text-sm mt-20">
              <Sparkles size={32} className="mx-auto mb-3 text-teal-500/50" />
        ...
        {pendingImage && (
          <div className="py-2 flex items-center gap-2 border-t border-[var(--border)] mt-3 pt-3">
            <img src={pendingImage.previewUrl} alt="Preview" className="w-12 h-12 rounded-lg object-cover border border-[var(--border)]" />
            <span className="text-xs text-teal-400 flex-1 truncate">{fileName} — ready to send</span>
            <button onClick={clearAttachment} className="text-xs text-[var(--text-muted)] hover:text-red-400 cursor-pointer">Remove</button>
          </div>
        )}
        ...
        <form onSubmit={sendMessage} className="flex items-center gap-2 pt-3 mt-1">
          <button type="button" onClick={() => fileInputRef.current?.click()} className="text-[var(--text-muted)] hover:text-[var(--text)] p-2 cursor-pointer" title="Attach a file">
            <Paperclip size={18} />
          </button>
          ...
          <button type="button" onClick={() => cameraInputRef.current?.click()} className="text-[var(--text-muted)] hover:text-[var(--text)] p-2 cursor-pointer" title="Take a picture">
            <Camera size={18} />
          </button>
          ...
          />
          <button type="submit" disabled={isLoading} className="bg-teal-600 hover:bg-teal-500 text-white p-2.5 rounded-xl transition cursor-pointer disabled:opacity-50">
            <Send size={16} />
          </button>
        </form>
      </div>
